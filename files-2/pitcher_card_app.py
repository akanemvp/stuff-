                    ...
                    if df_25 is None or len(df_25) == 0:
                        ...
                        with st.spinner("Scoring…"):
                            try:
                                ...
                                if "incompatible" in str(e) or "retrain" in str(e).lower():
                                    st.warning(
                                        "⚠️ The model was trained with an older version of the code. "
                                        "Go to the **Train Model** tab and click **🏋 Train / Retrain Model** "
                                        "to update it. This takes ~10 minutes."
                                    )
                                else:
    ...
    if st.button("🔍 Generate Card", type="primary"):
        ...
        with st.spinner("Scoring…"):
            try:
                ...
                if "incompatible" in str(e) or "retrain" in str(e).lower():
                    st.warning(
                        "⚠️ Model trained with older code. "
                        "Go to **Train Model** tab → **🏋 Train / Retrain Model**."
                    )
                else:
                    st.error(f"Scoring error: {e}")
                st.stop()
